...
import sys, os, glob, pickle, pylab as plt, struct, fit
...
coolgasf = filt.And(filt.LowPass('temp',1.5e4),filt.HighPass('rho','0.03 m_p cm^-3'))
...
re,re_err,nser,nser_err,Sigma=fit.sersic(h[i],filename=simname,linbin=0.2)
...
#             'mpseudob': np.sum(h[i].star[np.where(dec == 5)]['mass'].in_units('Msol')),
             ...
             'mgascool': np.sum(h[i].gas[coolgasf]['mass'].in_units('Msol')),
             ...
             'mdiskcoolgas': np.sum([h[i][diskf].g[coolgasf]['mass'].in_units('Msol')]),
             ...
             'mtwokpccoolgas': np.sum([h[i][twokpcf].g[coolgasf]['mass'].in_units('Msol')]),
             'mhalogas': mhalogas,
...
#             'Jtot':Jtot,'lambda':(Jtot / np.sqrt(2*np.power(mvir,3)*rvir*units.G)).in_units('1'),
             ...
             'sersic':{'re':re, 're_err':re_err, 'nser':nser, 'nser_err':nser_err, 'sigma':Sigma},
             'surfstar':{'r':sbprof['rbins'].in_units('kpc'), 
                   'sb':sbprof['sb,b'],'sv':sbprof['sb,v'],'si':sbprof['sb,i'],
                   'i_lum_den':sbprof['i_lum_den'],'v_lum_den':sbprof['v_lum_den'],'rho':sbprof['rho'],
...
try:
    ...
    pp.profile.density_soft_profile(h[i].dark,dmsoft,filename=simname+'.dmprof.png',center=False)
    plt.clf()
    ...
    pp.stars.satlf(h[i],filename=simname+'.satlf.png')
except:
    pass
